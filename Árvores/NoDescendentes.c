...
int ContaNo(NoArv *pai, int cont)
{
    if (pai->dir != NULL)
    {
        cont = 1 + ContaNo(pai->dir, cont);
    }
    if (pai->esq != NULL)
    {
        cont = 1 + ContaNo(pai->esq, cont);
    }
    return cont;
}

void Descendente(NoArv *pai, int num)
{
    int cont=0;
    if (Busca(pai, num)==0)
    {
    ...
    else
    {
        ...
        cont = ContaNo(pai, cont);
        printf("\nQuantidade de descendentes = %d", cont);
    }
}
